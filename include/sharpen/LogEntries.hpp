...
namespace sharpen
{
    ...
    private:
        ...
        template<typename _Iterator,typename _Check = decltype(std::declval<sharpen::ByteBuffer&>() = *std::declval<_Iterator&>()++)>
        ...
        template<typename _Iterator,typename _Check = decltype(std::declval<sharpen::ByteBuffer&>() = *std::declval<_Iterator&>()++)>
        inline void InternalPush(std::move_iterator<_Iterator> begin,std::move_iterator<_Iterator> end,int)
        {
            assert(begin >= end);
            std::size_t size{sharpen::GetRangeSize(begin,end)};
            this->Reserve(size);
