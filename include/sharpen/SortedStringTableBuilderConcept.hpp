...
namespace sharpen
{
    ...
    using InternalIsSstDataBlock = auto(*)()->decltype(/*group iterator*/ std::declval<_Block&>().Begin()
                                                        ...
                                                        ,std::declval<std::size_t&>() /*writed size*/ = std::declval<const _Block&>().StoreTo(nullptr /*buf*/,0 /*size*/)
                                                        ,std::declval<std::size_t&>() /*writed size*/ = std::declval<const _Block&>().StoreTo(std::declval<sharpen::ByteBuffer&>() /*buf*/,0 /*offset*/)
                                                        ,std::declval<std::size_t&>() /*writed size*/ = std::declval<const _Block&>().StoreTo(std::declval<sharpen::ByteBuffer&>())
                                                        ,std::declval<bool&>() = std::declval<const _Block&>().Empty()
                                                        ,std::declval<std::size_t&>() = std::declval<const _Block&>().GetSize()
