...
namespace sharpen {
    ...
    private:
        ...
        sharpen::MultiRaftForm BuildForm(sharpen::RaftMailType type,
                                         const sharpen::ByteBuffer &content) const noexcept;

        sharpen::Mail BuildMail(sharpen::RaftMailType type,
    ...
    public:
        ...
        virtual sharpen::Mail BuildVoteRequest(
            const sharpen::RaftVoteForRequest &request) const override;

        virtual sharpen::Mail BuildVoteResponse(
            const sharpen::RaftVoteForResponse &response) const override;

        virtual sharpen::Mail BuildHeartbeatRequest(
            const sharpen::RaftHeartbeatRequest &request) const override;

        virtual sharpen::Mail BuildHeartbeatResponse(
            const sharpen::RaftHeartbeatResponse &response) const override;

        virtual sharpen::Mail BuildPrevoteRequest(
            const sharpen::RaftPrevoteRequest &request) const override;

        virtual sharpen::Mail BuildPrevoteResponse(
            const sharpen::RaftPrevoteResponse &response) const override;

        virtual sharpen::Mail BuildSnapshotRequest(
            const sharpen::RaftSnapshotRequest &request) const override;

        virtual sharpen::Mail BuildSnapshotResponse(
            const sharpen::RaftSnapshotResponse &response) const override;
    };
}   // namespace sharpen
