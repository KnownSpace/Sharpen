...
namespace sharpen {
    ...
    private:
        ...
        constexpr static auto Test(int) noexcept
    ...
    public:
        using Type =
            sharpen::BoolType<!std::is_same<decltype(Self::Test<_Fn>(0)), TestFalse>::value>;
    };
    ...
    public:
        ...
        using Type =
            sharpen::BoolType<Value && sharpen::InternalMultiMatches<_Matches, _Args...>::Value>;
    };
    ...
    template<typename _Fn, typename... _Args>
    using InternalIsCompletedBindable = auto(*)()
                                            -> decltype(std::bind(std::declval<_Fn>(),
                                                                  std::declval<_Args>()...)());

    template<typename _Fn, typename... _Args>
    using IsCompletedBindable =
        sharpen::IsMatches<sharpen::InternalIsCompletedBindable, _Fn, _Args...>;

    template<typename _R, typename _Fn, typename... _Args>
    using InternalIsCallableReturned =
        auto(*)() -> sharpen::EnableIf<
            std::is_same<_R, decltype(std::declval<_Fn>()(std::declval<_Args>()...))>::value>;

    template<typename _R, typename _Fn, typename... _Args>
    using IsCallableReturned =
        sharpen::IsMatches<sharpen::InternalIsCallableReturned, _R, _Fn, _Args...>;

    template<typename _R, typename _Fn, typename... _Args>
    using InternalIsCompletedBindableReturned = auto(*)() -> sharpen::EnableIf<std::is_same<
        _R,
        decltype(std::bind(std::declval<_Fn>(), std::declval<_Args>()...)())>::value>;

    template<typename _R, typename _Fn, typename... _Args>
    using IsCompletedBindableReturned =
