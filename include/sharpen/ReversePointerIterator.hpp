...
namespace sharpen
{
    ...
    class ReversePointerIterator
        : public sharpen::
              IteratorTemplate<std::random_access_iterator_tag, _T, std::ptrdiff_t, _T *, _T &>
    {
    private:
    ...
    public:
        ...
        template<typename _NewT,
                 typename _Check = decltype(std::declval<_T *&>() == std::declval<_NewT *&>())>
        inline std::int32_t
        CompareWith(const sharpen::ReversePointerIterator<_NewT> &other) const noexcept
        {
    ...
    template<typename _T>
    inline sharpen::ReversePointerIterator<_T> operator+(sharpen::ReversePointerIterator<_T> ite,
                                                         std::ptrdiff_t value)
    {
        return ite += value;
    }
    ...
    template<typename _T>
    inline sharpen::ReversePointerIterator<_T> operator+(std::ptrdiff_t value,
                                                         sharpen::ReversePointerIterator<_T> ite)
    {
        return ite += value;
    }
