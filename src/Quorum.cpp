...
std::unique_ptr<sharpen::Broadcaster> sharpen::Quorum::NviCreateBroadcaster(
    std::size_t pipeline) const {
    assert(pipeline > 0);
    ...
    std::unique_ptr<sharpen::Broadcaster> broadcaster{new (std::nothrow) sharpen::Broadcaster{
        std::make_move_iterator(actors.begin()), std::make_move_iterator(actors.end()), pipeline}};
    if (!broadcaster) {
        throw std::bad_alloc{};
    }
    return broadcaster;
