...
void sharpen::PosixOutputPipeChannel::DoSafeClose(sharpen::ErrorCode err,
...
void sharpen::PosixOutputPipeChannel::RequestWrite(const char *buf,
                                                   std::size_t bufSize,
                                                   sharpen::Future<std::size_t> *future) {
    using FnPtr = void (*)(sharpen::EventLoop *, sharpen::Future<std::size_t> *, ssize_t);
    Callback cb =
        std::bind(static_cast<FnPtr>(&sharpen::PosixOutputPipeChannel::CompleteWriteCallback),
                  this->loop_,
                  future,
                  std::placeholders::_1);
    this->loop_->RunInLoop(
        std::bind(&sharpen::PosixOutputPipeChannel::TryWrite, this, buf, bufSize, std::move(cb)));
}

void sharpen::PosixOutputPipeChannel::WriteAsync(const char *buf,
                                                 std::size_t bufSize,
                                                 sharpen::Future<std::size_t> &future) {
...
void sharpen::PosixOutputPipeChannel::WriteAsync(const sharpen::ByteBuffer &buf,
...
void sharpen::PosixOutputPipeChannel::CompleteWriteCallback(sharpen::EventLoop *loop,
                                                            sharpen::Future<std::size_t> *future,
