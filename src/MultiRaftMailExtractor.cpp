...
sharpen::MultiRaftMailExtractor::MultiRaftMailExtractor(std::uint32_t magic,
    ...
    if (mail.Header().GetSize() == sizeof(sharpen::GenericMailHeader))
    {
        ...
        if (header.GetMagic() == this->magic_)
        {
            ...
            return form.CheckMagic() && form.GetType() != sharpen::RaftMailType::Unknown &&
                   form.GetRaftNumber() == this->raftNumber_;
        }
    }
    return false;
}

sharpen::RaftMailType sharpen::MultiRaftMailExtractor::NviGetMailType(
...
bool sharpen::MultiRaftMailExtractor::CheckMail(sharpen::RaftMailType expect,
                                                const sharpen::Mail &mail) const noexcept
{
    assert(mail.Header().GetSize() == sizeof(sharpen::GenericMailHeader));
    const sharpen::GenericMailHeader &header{mail.Header().As<sharpen::GenericMailHeader>()};
...
sharpen::Optional<sharpen::RaftVoteForRequest>
sharpen::MultiRaftMailExtractor::NviExtractVoteRequest(const sharpen::Mail &mail) const noexcept
{
...
sharpen::Optional<sharpen::RaftVoteForResponse>
sharpen::MultiRaftMailExtractor::NviExtractVoteResponse(const sharpen::Mail &mail) const noexcept
{
...
sharpen::MultiRaftMailExtractor::NviExtractHeartbeatRequest(
    const sharpen::Mail &mail) const noexcept
{
...
sharpen::MultiRaftMailExtractor::NviExtractHeartbeatResponse(
    const sharpen::Mail &mail) const noexcept
{
...
sharpen::Optional<sharpen::RaftPrevoteRequest>
sharpen::MultiRaftMailExtractor::NviExtractPrevoteRequest(const sharpen::Mail &mail) const noexcept
{
...
sharpen::MultiRaftMailExtractor::NviExtractPrevoteResponse(const sharpen::Mail &mail) const noexcept
{
...
sharpen::MultiRaftMailExtractor::NviExtractSnapshotRequest(const sharpen::Mail &mail) const noexcept
{
...
sharpen::MultiRaftMailExtractor::NviExtractSnapshotResponse(
    const sharpen::Mail &mail) const noexcept
{
