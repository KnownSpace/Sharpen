...
bool sharpen::GenericMailPaser::Completed() const noexcept {
    ...
    return this->parsedSize_ == sizeof(sharpen::GenericMailHeader) +
                                    this->header_.As<sharpen::GenericMailHeader>().GetContentSize();
}
...
void sharpen::GenericMailPaser::NviParse(sharpen::ByteSlice slice) {
    ...
    if (this->parsedSize_ < sizeof(sharpen::GenericMailHeader)) {
        std::size_t size{
            (std::min)(slice.GetSize(), sizeof(sharpen::GenericMailHeader) - this->parsedSize_)};
