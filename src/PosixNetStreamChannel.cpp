...
void sharpen::PosixNetStreamChannel::RequestRead(char *buf,std::size_t bufSize,sharpen::Future<std::size_t> *future)
{
...
void sharpen::PosixNetStreamChannel::RequestWrite(const char *buf,std::size_t bufSize,sharpen::Future<std::size_t> *future)
{
    using FnPtr = void(*)(sharpen::EventLoop *,sharpen::Future<std::size_t> *,ssize_t);
    Callback cb = std::bind(static_cast<FnPtr>(&sharpen::PosixNetStreamChannel::CompleteIoCallback),this->loop_,future,std::placeholders::_1);
...
void sharpen::PosixNetStreamChannel::RequestSendFile(sharpen::FileHandle handle,std::uint64_t offset,std::size_t size,sharpen::Future<std::size_t> *future)
{
    ...
    p += over;
    using FnPtr = void (*)(sharpen::EventLoop *,sharpen::Future<std::size_t> *,void *,std::size_t,ssize_t);
...
void sharpen::PosixNetStreamChannel::CompleteIoCallback(sharpen::EventLoop *loop,sharpen::Future<std::size_t> *future,ssize_t size) noexcept
{
    if (size == -1)
    {
        sharpen::ErrorCode code{sharpen::GetLastError()};
        if(code == sharpen::ErrorCancel || code == sharpen::ErrorConnectionAborted || code == sharpen::ErrorConnectionReset)
        {
...
void sharpen::PosixNetStreamChannel::CompleteSendFileCallback(sharpen::EventLoop *loop,sharpen::Future<std::size_t> *future,void *mem,std::size_t memLen,ssize_t size) noexcept
{
    ::munmap(mem,memLen);
    if (size == -1)
    {
...
void sharpen::PosixNetStreamChannel::WriteAsync(const sharpen::ByteBuffer &buf,std::size_t bufferOffset, sharpen::Future<std::size_t> &future)
{
...
void sharpen::PosixNetStreamChannel::ReadAsync(sharpen::ByteBuffer &buf,std::size_t bufferOffset, sharpen::Future<std::size_t> &future)
{
    if (buf.GetSize() < bufferOffset)
    {
        throw std::length_error("buffer size is wrong");
...
void sharpen::PosixNetStreamChannel::SendFileAsync(sharpen::FileChannelPtr file, std::uint64_t size, std::uint64_t offset, sharpen::Future<std::size_t> &future)
{
    if (!this->IsRegistered())
    {
        throw std::logic_error("should register to a loop first");
...
void sharpen::PosixNetStreamChannel::SendFileAsync(sharpen::FileChannelPtr file, sharpen::Future<std::size_t> &future)
{
