...
sharpen::DnsResolveResult sharpen::Dns::ConvertAddrInfoToResolveResult(addrinfo *addrInfo)
{
    ...
    r.SetAddressFamily(addrInfo->ai_family == AF_INET ? sharpen::AddressFamily::Ip:sharpen::AddressFamily::Ipv6);
    if(addrInfo->ai_canonname)
    {
        r.Canonname() = sharpen::ByteBuffer{addrInfo->ai_canonname,std::strlen(addrInfo->ai_canonname)};
    }
...
#endif
        r.EndPointPtr().reset(new (std::nothrow) sharpen::IpEndPoint{addr,::ntohs(paddr->sin_port)});
        if(!r.EndPointPtr())
        {
            throw std::bad_alloc{};
        }
    ...
    else
    {
        ...
        r.EndPointPtr().reset(new (std::nothrow) sharpen::Ipv6EndPoint{paddr->sin6_addr,::ntohs(paddr->sin6_port)});
        if(!r.EndPointPtr())
        {
            throw std::bad_alloc{};
        }
